...
- Status: Alpha — v0.9.2 (stability release: crash fixes, data integrity, sync reliability, 955 tests).
...
- **sqlite3 Android liblog patch**: The pre-built SQLCipher binaries shipped by `sqlite3` reference `__android_log_vprint` from Android’s `liblog.so`, but the dynamic linker can’t resolve it when loading the bundled `.so`. Two-pronged fix: (1) `dependency_overrides` in `pubspec.yaml` forces `sqlite3 >= 3.5.2` (SQLCipher 4.18.0, which builds the binary with proper `liblog` linkage). (2) `tool/patch_sqlite3_android_log.dart` adds `'log'` to the Android libraries list in the sqlite3 build hook for the compile-from-source path (F-Droid builds). CI runs it after `flutter pub get`. **Remove once upstream `sqlite3` links `liblog` on Android by default.**
- **F-Droid compile-from-source OpenSSL**: The `sqlite3` hook's `source: source` path compiles the SQLCipher amalgamation but links **no crypto library**, so the F-Droid/CI APKs crashed at `PRAGMA key` with an unresolved `RAND_bytes`. The fix, in the CI `build-android` job only: (1) a `Build OpenSSL` step cross-compiles OpenSSL 3.4.1 (`no-shared`) per ABI into `/opt/openssl-android/{android-arm64,android-arm,android-x86_64}` (static `libcrypto.a` + headers); (2) `tool/patch_fdroid_sqlite.dart <sqlcipher> <openssl-dir>` rewrites pubspec to `source: source`, deletes the old stub OpenSSL headers, and idempotently patches the sqlite3 build hook so the amalgamation **compiles against the real OpenSSL headers for every target config** (Android and the Linux host build alike — headers are arch-independent) and **statically links the per-ABI `crypto` lib on Android only**, selected from `input.config.code.targetArchitecture`. The OpenSSL path is baked into the hook, so hooks don't need the `NOOK_OPENSSL_DIR` env at build time. The script also **purges `.dart_tool/hooks_runner`/`.dart_tool/native_assets`** afterwards: `flutter pub get` compiles each hook to a cached `hook.dill` that editing `build.dart` does *not* invalidate, so a stale (unpatched) hook would otherwise keep running. A post-build CI step unzips each APK's `libsqlite3.so` and asserts `RAND_bytes` is a **defined** symbol, so a silent regression fails CI. Local developers never hit this (default `source: sqlcipher` uses prebuilt binaries with OpenSSL statically linked); the hook patch lives only in the ephemeral CI pub cache.

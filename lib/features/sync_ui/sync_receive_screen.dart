...
class _SyncReceiveScreenState extends ConsumerState<SyncReceiveScreen>
  ...
  Future<void> _scanAndReceive(BuildContext context) async {
    ...
    // Both devices verify the same 6-digit code before any data moves. The
    // pairing screen stays open on both devices until BOTH accept the code.
      ...
      MaterialPageRoute(
        builder: (_) => SyncPairingScreen(
          ...
          onConfirm: () async {
            ...
            // The orchestrator lands in the error phase when the receiver
            // rejected, timed out, or the dial failed.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Center(
          // Cap the width so the beacon + controls don't stretch absurdly on
          // large displays (desktop/tablet); keep the phone layout intact.
                      ...
                      child: Column(
                        ...
                        children: [
                                  ...
                                  AnimatedBuilder(
                                    ...
                                    builder: (context, child) {
                                      return Transform.scale(
                                        scale: 1.0 +
                                            (_broadcastController.value * 1.5),
                                        child: Opacity(
                                          opacity:
                                              1.0 - _broadcastController.value,
                                          child: Container(
                                            width: 120,
                                            height: 120,
                          ...
                          if (qrCameraSupported) ...[
                            ...
                            // Prominent "scan a desktop's QR" action — surfaced here
                            // so a phone receiving from a PC doesn't have to scroll
                            // to the bottom of the screen to find it.
                            FilledButton.icon(
                              ...
                              label: const Text('Scan a sender\'s QR code'),

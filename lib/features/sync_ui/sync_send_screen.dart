...
class _SyncSendScreenState extends ConsumerState<SyncSendScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(
        // Cap the width so the radar + payload don't stretch absurdly on large
        // displays (desktop/tablet); keep the phone layout intact.
  ...
  Future<void> _connectAndSend(
    ...
    // The pairing screen is shown on BOTH devices at the same time with the
    // same code. Tapping Confirm on this side dials the receiver (onConfirm)
    // but keeps the PIN visible in a "waiting" state until the receiver
    ...
    final confirmed = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => SyncPairingScreen(
          ...
          onConfirm: () async {
            await notifier.connectToDevice(dialDevice,
                pairingCode: pairingCode);
